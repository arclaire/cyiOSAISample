...
- (void)layoutViews {
    ...
    offset -= self.rulerCollectionView.frame.size.width / 2.0;
    //校正偏差
    ...
    /*
    CGFloat limitValue = 20;
    self.rulerCollectionView.contentSize = (CGSize){(limitValue + limitValue) * 10 * (self.scaleWidth + self.distanceBetweenScale) + self.scaleWidth, self.height};
     */
}
...
- (void)reset {
    ...
    if (activeSelectionNumber >= 0) {
        if (self.isDecimal) {
            //偏移计算：(单个刻度尺宽度 + 刻度尺间距) * 总刻度 - 起始偏移 + 最后一个刻度宽度 / 2.0
            offset = activeSelectionNumber * 10 * (self.scaleWidth + self.distanceBetweenScale)  + (self.scaleWidth / 2.0);
            //检测数字是否符合条件
            NSString *defaultValue = [NSString stringWithFormat:@"%lf", activeSelectionNumber * 10];
            if ([RulerView isPureInt:defaultValue]) {
                self.selectIndex = activeSelectionNumber * 10;
            ...
            //偏移计算：(单个刻度尺宽度 + 刻度尺间距) * 总刻度 - 起始偏移 + 最后一个刻度宽度 / 2.0
            offset = activeSelectionNumber * (self.scaleWidth + self.distanceBetweenScale) + (self.scaleWidth / 2.0);
            self.selectIndex = activeSelectionNumber;
        }
    }
    ...
    [self.rulerCollectionView setContentOffset:CGPointMake(offset, 0) animated:YES];
}
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    if ([self.delegate respondsToSelector:@selector(rulerWillBeginDragging:)]) {
        [self.delegate rulerWillBeginDragging:self];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if ([self.delegate respondsToSelector:@selector(rulerDidEndDragging:)]) {
        [self.delegate rulerDidEndDragging:self];
    }
}
...
- (void)selectCell {
    if(!self.showSelectedValue) return;
...
- (void)addStartGradientLayer {
    ...
    //设置颜色数组
    self.startGradientLayer.colors = @[(__bridge id)[UIColor colorWithWhite:0.0 alpha:0.8].CGColor,
                                       (__bridge id)[UIColor colorWithWhite:0.0 alpha:0.2].CGColor];
...
- (void)addEndGradientLayer {
    ...
    //设置颜色数组
    self.endGradientLayer.colors = @[(__bridge id)[UIColor colorWithWhite:0.0 alpha:0.2].CGColor,
